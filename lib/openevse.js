...
const debug = require("debug")("openevse");
...
class OpenEVSE extends EventEmitter
{
  ...
  _checksum(msg)
  {
    ...
    var checkString = this._toHex(check, 2);
  ...
  {
    var command = this._checksum("$" + (Array.isArray(args) ? args.join(" ") : args));
  ...
  _processCommands()
  {
    ...
    this._driver.rapi(command.command, (data) => {
      debug("Got '"+data+"'");
      ...
      .error((err) => { command.request._error(err); })
      .always(() => { command.request._always(); });
  }
  ...
  version(callback) {
    var request = this._request("GV", (data) => {
      if(data.length >= 2) {
        ...
        callback(version, protocol, this._version);
      } else {
        request._error(new OpenEVSEError("ParseError", "Only received "+data.length+" arguments"));
      }
    });
   ...
   *
   */
  lcd_claim(callback, claim) {
    var request = this._request(["F0", claim ? 0 : 1], () => {
      callback();
    });
    return request;
  }
   ...
   *
   */
  lcd_set_backlight(callback, color) {
    var colorNumber = Number.isInteger(color) ? color : this._lcd_colors.indexOf(color);
    var request = this._request(["FB", colorNumber], () => {
      callback();
    });
    return request;
  }
   ...
   *
   */
  lcd_print_text(callback, x, y, text) {
    var request = this._request(["FP", x, y, text], () => {
      callback();
    });
    return request;
  }
...
exports.connect = function(endpoint)
{
  ...
  if("simulator" === endpoint) {
    const OpenEVSEDriverSimulator = require("./OpenEVSEDriverSimulator");
    driver = new OpenEVSEDriverSimulator();
  } else if(endpoint.startsWith("http:") || endpoint.startsWith("https:")) {
    const OpenEVSEDriverHttp = require("./OpenEVSEDriverHttp");
    driver = new OpenEVSEDriverHttp(endpoint);
  } else {
    const OpenEVSEDriverSerial = require("./OpenEVSEDriverSerial");
    driver = new OpenEVSEDriverSerial(endpoint);
  }

  debug("Connected to "+endpoint+" using "+driver.constructor.name);
